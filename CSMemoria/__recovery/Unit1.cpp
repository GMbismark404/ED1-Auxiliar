...
//		posX+=TamanoCeldaX;
		...
		for(int i = 0; i < max_memoria; i++){
			...
			posX+=TamanoCeldaX;
			//Dato
			PintadoConBorde(posX,posY,IntToStr(a->getDato(i)),clLime);
			posX+=TamanoCeldaX;
			//ID
			PintadoConBorde(posX,posY,a->getId(i).c_str(),clYellow);
			posX+=TamanoCeldaX;
			//LINK
			PintadoConBorde(posX,posY,IntToStr(a->getLink(i)),clYellow);
			posX=auxX;
			posY+=TamanoCeldaY;
		}
